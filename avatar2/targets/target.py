...
def synchronize_state(*states, **kwargs):
    ...
    :type states: list
    :param kwargs: If it contains "transition_optional" it also allow to return
                   if the target is already in the desired states, even if the
    ...
    def decorator(func):
        ...
        def state_synchronizer(self, *args, **kwargs):
            ...
            if len(states) == 1:
                ...
                ret = synchronize_state(*states[:-1])(func)(self, *args,
                                                            **kwargs)
            if blocking is True:
...
class TargetProtocolStore(object):
    ...
    def __init__(self, additional_protocols=None):
        ...
        # Stores protocol references and their count
    ...
    def set_all(self, instance, only_defaults=False):
        ...
        :param only_defaults: Use only the default protocols or all of them
        """
    ...
    def shutdown(self):
        """Shuts down all the associated protocols"""
        for p in self.protocols:
            setattr(self, p, None)
    ...
    def __setattr__(self, name, value):
        ...
        if value is not None and self.unique_protocols.get(value,
                                                           None) is None:
...
class Target(object):
    ...
    def __init__(self, avatar, name=None):  # type: ('Avatar', str) -> None
        ...
        :param avatar: The avatar instance this target should be orchestrated
                       by
        :param name: The name of this target, mainly for logging. This is
                     optional and will be autogenerated otherwise.
        """
        ...
        self.log = logging.getLogger(
            '%s.targets.%s' % (avatar.log.name, self.name))
        log_file = logging.FileHandler(
            '%s/%s.log' % (avatar.output_directory, self.name))
    ...
    def set_breakpoint(self, line, hardware=False, temporary=False,
        ...
        :param line: The line at which the the breakpoint will be inserted
        ...
        :param int thread: Thread-no in which this breakpoints should be added
        """
